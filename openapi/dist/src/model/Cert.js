...
(function(root, factory) {
   ...
   * @param keyAlg {String} Private key algorithm, for example RSA or EC.
   * @param keyLen {Number} Private key length, for example 256 for an ECDSA key or 2048 for an RSA key.
   * @param keyStrength {Number} Estimated private key strength in symmetric bits. For example, a 256-bit ECDSA key is thought to provide 128 bits of security, whereas a 2048-bit RSA key provides about 112. 
   * @param sigAlg {module:model/Cert.SigAlgEnum} Certificate signature algorithm.
   * @param spkiSha256 {String} Hex-encoded SHA256 of the certificate's SPKI component
   ...
   * @param caPathLen {Number} Indicates if this is a CA certificate. Leaf certificates will have -1 in this field, indicating that they cannot issue further certificates. CA certificates will have 0 or a positive number. CA certificates without path length limit will have 2,147,483,647. 
   * @param wildcard {Boolean} Set to true if the hostname list contains at least one wildcard.
   * @param selfSigned {Boolean} Set to true if this is a self-signed certificate.
   ...
   * @param seenInstalled {Boolean} Indicates if this certificate has been seen installed on a server
   * @param active {Boolean} Indicates if this certificate has been seen installed recently
   * @param precert {Boolean} Indicates if the returned certificate is, in fact, a precertificate.
   * @param managed {Boolean} Indicates if this certificate is managed, i.e. provided by a third-party service that will also automatically renew the certificate when it becomes due. 
   * @param ownership {module:model/Cert.OwnershipEnum} Indicates if this certificate belongs to the organisation, which means that it is served from one of the organization's properties. Do note that, in the case of certificate name mismatch, someone else's certificate may be considered as own. In that case, the **matchesOrgHost** field will be set to false. 
   * @param matchesOrgHost {Boolean} Indicates if at least one of the host listed in the certificates matches a hosts that belongs to the organisation. 
   * @param symantecStatus {module:model/Cert.SymantecStatusEnum} Indicates if this certificate was issued from Symantec's PKI infrastructure. 
   * @param ctLogs {Array.<module:model/CertCtLog>} 
   * @param pem {String} Base64-encoded certificate in DER format
   */
  var exports = function(sha256, subject, serial, issuer, notBefore, notAfter, effectiveNotAfter, revokedOn, keyAlg, keyLen, keyStrength, sigAlg, spkiSha256, hosts, caPathLen, wildcard, selfSigned, firstSeen, lastSeen, seenInstalled, active, precert, managed, ownership, matchesOrgHost, symantecStatus, ctLogs, pem) {
    var _this = this;
    ...
    _this['keyAlg'] = keyAlg;
    _this['keyLen'] = keyLen;
    _this['keyStrength'] = keyStrength;
    _this['sigAlg'] = sigAlg;
    _this['spkiSha256'] = spkiSha256;
    _this['hosts'] = hosts;
    _this['caPathLen'] = caPathLen;
    _this['wildcard'] = wildcard;
    _this['selfSigned'] = selfSigned;
    _this['firstSeen'] = firstSeen;
    _this['lastSeen'] = lastSeen;
    _this['seenInstalled'] = seenInstalled;
    _this['active'] = active;
    _this['precert'] = precert;
    _this['managed'] = managed;
    _this['ownership'] = ownership;
    _this['matchesOrgHost'] = matchesOrgHost;
    _this['symantecStatus'] = symantecStatus;
  ...
  /**
   * Indicates if this certificate has been seen installed recently
